...
> A complete guided tour of the `example-apps/store/thane/` source code — a small product-catalogue-and-cart application that exercises routing, signals, data fetching, localStorage persistence, and every major directive.
    ...
    template: html`
      <nav class="navbar">
        <a class="logoLink" @click=${navigate('/')}>
          <img class="logo" src=${logoPath} alt="Store logo" />
        </a>

...
export const HomePage = defineComponent('home-page', ({ root }) => ({
    ...
    <div class="home-page">
      ...
      <section class="hero section">
        <div class="container hero-inner">
          <a href="https://github.com/timlouw/thane" target="_blank" rel="noopener noreferrer" class="badge hero-badge">
            ...
            Open Source
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="12" height="12"><path d="M7 17L17 7M17 7H7M17 7V17"/></svg>
          </a>
          ...
          <div class="hero-actions">
            ...
            <a href="https://github.com/timlouw/thane" target="_blank" rel="noopener noreferrer" class="btn btn-secondary">
      ...
      <section class="section comparison-section">
        <div class="container">
          <div class="section-header">
            ...
            <p>The compiler traces every binding at build time and generates the exact DOM operations needed — nothing more.</p>
          </div>
      ...
      <section class="section features-section">
        <div class="container">
          ...
          <div class="grid grid-3">
            ${FeatureCard({ icon: '🔬', title: 'Compile-Time Optimized', description: 'HTML templates are pre-compiled into static <template> elements with direct DOM path navigation. No runtime parsing ever.' })}
            ${FeatureCard({ icon: '⚡', title: 'Fine-Grained Reactivity', description: 'Signal-based subscriptions at the individual binding level. Only the exact text node, attribute, or style that changed is updated.' })}
            ${FeatureCard({ icon: '🪶', title: 'Tiny Runtime', description: 'The entire runtime is ~3KB min+gzip. Most logic runs at compile time. Just signals, a reconciler, and mount.' })}
            ${FeatureCard({ icon: '🌐', title: 'Light DOM', description: 'No Shadow DOM. Components render as regular DOM elements with auto-scoped CSS. Natural cascade, standard devtools.' })}
            ${FeatureCard({ icon: '📦', title: 'Built-in Directives', description: 'when(), whenElse(), and repeat() with keyed reconciliation, empty-state fallbacks, and full nesting support.' })}
            ${FeatureCard({ icon: '🛡️', title: 'TypeScript-First', description: 'Written in TypeScript, ships declarations, full IDE autocompletion. 12 compile-time lint rules catch silent failures.' })}
          </div>
        </div>
      </section>
      ...
      <section class="section">
        <div class="container">
          ...
          <div class="primitives-layout">
            <div class="primitives-code">
              ${CodeBlock({ code: REACTIVE_EXAMPLE, lang: 'typescript' })}
            </div>
      ...
      <section class="section">
        <div class="container">
          <div class="section-header">
            <h2>Try it live</h2>
            <p>Edit the code below and see instant results. This embedded playground runs a mini Thane runtime in a sandboxed iframe.</p>
          </div>
      ...
      <section class="section">
        <div class="container">
          ...
          <div class="comparison-grid">
            <div class="comparison-col">
              ${CodeBlock({ code: REPEAT_EXAMPLE, lang: 'typescript' })}
            </div>
            <div class="comparison-col directives-info">
              <div class="directive-item">
                <code>when()</code>
                <p>Show/hide elements based on a reactive condition. The element is removed from the DOM when hidden.</p>
              </div>
              <div class="directive-item">
                <code>whenElse()</code>
        ...
        <div class="container">
          ...
          <div class="faq-list">
            <details>
              ...
              <div>
                <p>Thane is a compile-time component framework for building web applications. It compiles your declarative TypeScript components into optimized vanilla JavaScript at build time — template cloning, direct DOM navigation, and fine-grained signal subscriptions — so the browser does the absolute minimum work at runtime.</p>
              </div>
            </details>
            <details>
              <summary>How does compile-time compilation work?</summary>
              <div>
                <p>The Thane compiler finds defineComponent() calls, extracts html and css tagged templates via AST analysis, identifies all bindings, and generates the exact DOM operations needed. Each signal read becomes a direct subscription to update a specific text node, attribute, or style. No virtual DOM, no diffing.</p>
              </div>
            </details>
            <details>
              <summary>Do I need a build step?</summary>
              <div>
                <p>Yes. Thane's power comes from compile-time optimization. The CLI provides dev (with hot reload), build, and serve commands. Under the hood it uses esbuild for fast bundling.</p>
              </div>
            </details>
            <details>
              <summary>How small is the runtime?</summary>
              <div>
                <p>The runtime is approximately 3KB min+gzip. It includes signal(), computed(), effect(), batch(), defineComponent(), and a keyed reconciler. Most of the logic runs at compile time, not in the browser.</p>
              </div>
            </details>
            <details>
              <summary>Can I use Thane with existing projects?</summary>
              <div>
                <p>Thane is designed as a standalone framework with its own build pipeline. You can gradually adopt it by mounting Thane components into specific sections of an existing page.</p>
              </div>
            </details>
            <details>
              <summary>What browsers are supported?</summary>
              <div>
                <p>Thane targets modern evergreen browsers: Chrome 120+, Firefox 117+, Safari 17.2+, and Edge 120+. The compiled output uses standard DOM APIs with no polyfills required.</p>
              </div>
            </details>
            <details>
              <summary>Is Thane production ready?</summary>
              <div>
                <p>Thane is currently pre-1.0 and under active development. It has 158+ unit tests and comprehensive end-to-end browser tests across Chromium, Firefox, and WebKit. Check the releases page for the latest version.</p>
              </div>
            </details>
          </div>
        </div>
  ...
  styles: css`
    ...
    .cta-inner h2 { position: relative; margin-bottom: 12px; }
    .cta-inner p { position: relative; margin-bottom: 24px; font-size: 1.05rem; }

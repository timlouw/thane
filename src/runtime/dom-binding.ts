 ...
 * Comment markers are inserted as <!--id--> before text nodes for mixed content scenarios
 * 
 * @param elements - Array of elements to search within
 * @param id - The comment marker ID to find
 * @returns The text node following the comment, or null if not found
 */
export const __findTextNode = (elements: Element[], id: string): Text | null => {
  ...
  const walkNodes = (node: Node): Text | null => {
    ...
    if (node.nodeType === Node.COMMENT_NODE && node.textContent === searchComment) {
      // The text node should be the next sibling
      const nextSibling = node.nextSibling;
      if (nextSibling && nextSibling.nodeType === Node.TEXT_NODE) {
        return nextSibling as Text;
      }
      // If next sibling isn't a text node, create one

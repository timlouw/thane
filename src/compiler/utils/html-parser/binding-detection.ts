...
import { WHEN_ELSE_REGEX, REPEAT_REGEX, SIGNAL_EXPR_REGEX } from './types.js';
...
/** Options for controlling binding detection behaviour */
export interface BindingDetectionOptions {
  /** When true, create bindings for non-signal expressions (e.g. product.image).
   *  Only appropriate for component-level templates (module scope). */
  detectNonSignalBindings?: boolean;
}
...
export function findBindingsInText(
  ...
  options?: BindingDetectionOptions,
): void {
  if (!parent) return;
  ...
  while (searchPos < text.length) {
    ...
    } else if (options?.detectNonSignalBindings && exprContent.trim()) {
      // Non-signal expression (e.g. prop access like product.title) — treat as static expression binding
      bindings.push({
        element: parent,
        type: 'text',
        signalName: '',
...
export function findBindingsInAttributes(
  element: HtmlElement,
  bindings: BindingInfo[],
  options?: BindingDetectionOptions,
): void {
  ...
  for (const [name, attr] of element.attributes) {
    ...
    if (name === 'style') {
      ...
      for (const expr of styleExprs) {
        ...
        } else if (options?.detectNonSignalBindings) {
          // Non-signal style expression (e.g. prop-derived value)
          bindings.push({
            element,
            type: 'style',
            signalName: '',
    ...
    for (const expr of attrExprs) {
      ...
      if (signals.length > 0) {
        ...
        // Non-signal attribute expression (e.g. prop-derived value like product.image)
        bindings.push({
          element,
          type: 'attr',
          signalName: '',

...
const resolveSelectorFromImport = async (
...
const generateFileContent = (generatedFilePath: string, routesFilePath: string, selectorEntries: Array<[string, string]>): string => {
  const routesImportSpecifier = getImportSpecifier(generatedFilePath, routesFilePath);
  const selectorLines =
    selectorEntries.length > 0
      ? selectorEntries.map(([selector, pattern]) => `    '${escapeTypeLiteral(selector)}': '${escapeTypeLiteral(pattern)}';`).join('\n')
...
export const syncProjectTypes = async (workspaceRoot: string = process.cwd()): Promise<number> => {
  ...
  const generatedFiles = (await fs.stat(generatedRoot).then(() => true).catch(() => false))
